...
export default async function VideoPage({
  ...
  const downloadUrl = `https://stream.mux.com/${playbackId}/high.mp4?download=screen-recording-${date}.mp4`;
    ...
    <main className="min-h-screen bg-slate-950 p-6 md:p-12 text-slate-200">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div
            id="video-container"
            className="bg-black rounded-2xl overflow-hidden shadow-2xl border border-slate-800 aspect-video relative"
          >
            {isVideoReady ? (
              <>
                <MuxPlayerWrapper playbackId={playbackId} />
                <FullscreenButton targetId="video-container" />
                {!isTranscriptReady && (
